...
def recv_zmq_message():
    ...
    if evt:
        if evt.get(socket) == zmq.POLLIN:
            ...
            print(response)
            response = val_toFitScreen(float(response))
            movingWindow = np.append(movingWindow, response)
            movingWindow = movingWindow[1:]
            dM = np.diff(movingWindow)

            mean_dM = np.append(mean_dM, (np.mean(dM)/dT))
            mean_dM = mean_dM[1:]
...
index = np.c_[np.repeat(np.repeat(np.arange(ncols), nrows), n),  ## np.repeat([0,0,1,1]) -----> array([0, 0, 0, ..., 1, 1, 1]) 500 500 times
...
class Canvas(app.Canvas):
    def __init__(self):
        ...
        app.Canvas.__init__(self, title=str(time.time()),
                            keys='interactive')

        self.program = gloo.Program(VERT_SHADER, FRAG_SHADER)
        self.program['a_position'] = recv_zmq_message().reshape(-1, 1).astype(np.float32)
        ...
        # self.testText = visuals.TextVisual('TestingTestingTestingTesting',pos=(20,20))
    ...
    def on_timer(self, event):
        ...
        global socket
        y = recv_zmq_message().reshape(-1,1)
