...
def recv_zmq_message(socket):
    ...
    if evt:
        if evt.get(socket) == zmq.POLLIN:
            ...
            response[0] = response[0].split("b'")[1]
            response[-1] = response[-1].rstrip("'")
            #print(response, type(response))
            response = [val_toFitScreen(float(i)) for i in response]
            if len(response) > n:
                diff = len(response) - n
...
class Canvas(app.Canvas):
    def __init__(self):
        ...
        self.program['a_position'] = recv_zmq_message(socket).reshape(-1, 1).astype(np.float32)
    ...
    def on_timer(self, event):
        ...
        global socket
        y = recv_zmq_message(socket).reshape(-1,1)
